...
import type { GenreStat } from '../../../shared/types/stats';
...
type GenreDonutChartDatum = {
    name: string;
    value: number;
};

type GenreTooltipProps = {
    active?: boolean;
    payload?: Array<{
        name?: string;
        value?: number;
...
const CustomTooltip = ({ active, payload, t }: GenreTooltipProps) => {
...
const GenreDonutChart = ({ genreDistribution = [] }: GenreDonutChartProps) => {
    const { t } = useTranslation();

    const data = useMemo<GenreDonutChartDatum[]>(() =>
        genreDistribution.map((genre) => ({
            name: t(`stats.genre.names.${genre.genre.toLowerCase()}`, { defaultValue: genre.genre }),
            value: genre.count,
        })),
        [genreDistribution, t]
    );
